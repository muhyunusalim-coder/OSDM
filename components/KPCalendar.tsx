...
export const KPCalendar: React.FC<Props> = React.memo(() => {
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 md:p-8 border border-slate-200/60 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden group">
            {/* Background elements */}
            <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-emerald-500/5 to-transparent rounded-full blur-[40px] -mr-20 -mt-20 transition-transform duration-700 group-hover:scale-110"></div>
            <div className="absolute bottom-0 left-0 w-full h-[3px] bg-gradient-to-r from-emerald-500 via-purple-500 to-pink-500 scale-x-0 group-hover:scale-x-100 transition-transform duration-700 origin-left"></div>
            ...
            <div className="relative z-10">
                <div className="mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-6">
                    <div>
                        <h2 className="text-xl md:text-2xl font-display font-bold text-slate-900 dark:text-slate-100 tracking-tight flex items-center gap-3">
                            <div className="p-2 bg-emerald-50 dark:bg-emerald-950/30 rounded-xl text-emerald-600 dark:text-emerald-400 border border-emerald-100/50 dark:border-emerald-800/50 shrink-0">
                ...
                <div className="relative pl-2 sm:pl-6">
                    {/* Vertical Line */}
                    <div className="absolute left-[23px] sm:left-[39px] top-4 bottom-8 w-[2px] bg-gradient-to-b from-emerald-200 via-purple-200 to-transparent rounded-full opacity-60"></div>
                                ...
                                {selectedMonth.activities.map((act, index) => (
                                    ...
                                    >
                                        ...
                                        <div className="relative z-10 flex flex-col items-center mt-1">
                                            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center bg-white dark:bg-slate-900 text-emerald-600 dark:text-emerald-400 font-bold text-xs sm:text-sm border-[3px] border-emerald-100/80 dark:border-emerald-800/60 shadow-sm group-hover/item:border-emerald-500 group-hover/item:text-emerald-700 dark:group-hover/item:text-emerald-300 transition-colors duration-300 shrink-0">
                                                {index + 1}
                                            </div>
                                            {/* Glow effect on hover */}
                                            <div className="absolute inset-0 rounded-full bg-emerald-400 blur-[8px] opacity-0 group-hover/item:opacity-35 transition-opacity duration-300 scale-125"></div>
                                        </div>
                                        
                                        {/* Card */}
                                        <div className="flex-1 p-4 md:p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all duration-300 group-hover/item:-translate-y-0.5 group-hover/item:shadow-md group-hover/item:border-emerald-200 dark:group-hover/item:border-emerald-800 relative overflow-hidden">
                                            {/* Subtle Left Border Highlight */}
                                            <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-emerald-400 to-purple-400 scale-y-0 group-hover/item:scale-y-100 transition-transform duration-300 origin-top"></div>
                                            
                                            <div className="flex items-center gap-2 mb-2">
                                                <div className="p-1 bg-emerald-50 dark:bg-emerald-950/40 rounded text-emerald-600 dark:text-emerald-400">
                                                    <Clock size={11} strokeWidth={2.5} />
                                                </div>
                                                <p className="text-[10px] md:text-[11px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">{act.range}</p>
                                            </div>
