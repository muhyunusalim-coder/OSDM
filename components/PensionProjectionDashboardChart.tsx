...
const PensionProjectionDashboardChart: React.FC<Props> = React.memo(({ employees, language }) => {
  ...
  const filteredRetirees = useMemo(() => {
    ...
    return activeYearData.list.filter(
      emp =>
            ...
            {activeYearData.count > 0 && (
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full text-xs pl-8.5 pr-3 py-1.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-400 dark:focus:border-rose-500 transition-all font-medium text-gray-700 dark:text-gray-200 placeholder:text-gray-400 dark:placeholder:text-gray-500"
                />
              </div>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto max-h-[220px] space-y-2 pr-1 custom-scrollbar">
            {filteredRetirees.length > 0 ? (
              filteredRetirees.map((emp) => (
                <div
                  key={emp.id}
                  className="bg-white dark:bg-gray-800 border border-gray-200/80 dark:border-gray-700 rounded-lg p-3 hover:border-rose-200 dark:hover:border-rose-700 transition-all duration-200 group/item flex flex-col gap-1 shadow-sm"
                >
