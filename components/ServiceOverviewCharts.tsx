...
const ServiceOverviewCharts: React.FC<Props> = React.memo(({ employees, promotionEmployees, stats, language }) => {
  ...
  const kgbData = useMemo(() => {
    ...
    filteredEmployees.forEach(emp => {
      ...
      else if ((emp.status as string) === 'Processing' || emp.status === 'Upcoming') counts['Proses']++;
  ...
  const kpData = useMemo(() => {
    ...
    promotionEmployees.forEach(emp => {
      ...
      if (emp.pangkat && (emp.pangkat.includes('IX') || emp.pangkat.includes('VII') || emp.pangkat.includes('V/'))) return;
  ...
  const ageDistData = useMemo(() => {
    ...
    return Object.entries(brackets).map(([range, Jumlah]) => ({ range, Jumlah }));
  }, [employees]);

  const ageColors = ['#10b981', '#3b82f6', '#8b5cf6', '#f59e0b', '#ef4444', '#b91c1c'];
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200/80 dark:border-slate-800 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden flex flex-col justify-between group">
         ...
         <div className="h-[250px] w-full relative z-10">
            <ResponsiveContainer width="100%" height="100%">
               <PieChart>
                 <Pie data={kgbData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={65} outerRadius={85} paddingAngle={5}>
                    {kgbData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} stroke="transparent" />)}
                 </Pie>
                 <Tooltip contentStyle={{ borderRadius: '1rem', border: 'none', boxShadow: '0 10px 20px -5px rgba(0,0,0,0.1)' }} />
         ...
         <div className="h-[250px] w-full relative z-10">
            <ResponsiveContainer width="100%" height="100%">
               <BarChart data={kpData}>
                 ...
                 <Tooltip cursor={{ fill: 'rgba(248, 250, 252, 0.1)' }} contentStyle={{ borderRadius: '1rem', border: '1px solid #f1f5f9', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.05)' }} />
                 <Bar dataKey="value" fill="#3b82f6" radius={[6, 6, 0, 0]} barSize={28}>
                    {kpData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[(index + 1) % COLORS.length]} />)}
                 </Bar>
               </BarChart>
            </ResponsiveContainer>
         </div>
      ...
      {/* Pensiun Card */}
      <div className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200/80 dark:border-slate-800 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden flex flex-col justify-between group">
         <div className="absolute top-0 right-0 w-40 h-40 bg-gradient-to-br from-pink-400/10 dark:from-pink-400/5 to-transparent rounded-full blur-[30px] -mr-10 -mt-10 transition-transform duration-700 group-hover:scale-125"></div>
         <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-[0.02] dark:opacity-[0.05] mix-blend-overlay pointer-events-none"></div>
         <div className="absolute bottom-0 left-0 w-full h-[3px] bg-gradient-to-r from-pink-400 to-rose-500 scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"></div>
         
         <h3 className="font-display font-medium text-slate-800 dark:text-slate-100 text-lg mb-6 tracking-tight relative z-10">{t('chart_pensiun_title')}</h3>
         <div className="h-[250px] w-full relative z-10">
            <ResponsiveContainer width="100%" height="100%">
               <AreaChart data={pensiunData}>
                 ...
                 <Tooltip contentStyle={{ borderRadius: '1rem', border: '1px solid #f1f5f9', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.05)' }} />
                 <Area type="monotone" dataKey="Pensiun" stroke="#ec4899" strokeWidth={3} fillOpacity={1} fill="url(#colorPensiun)" />
               </AreaChart>
            </ResponsiveContainer>
         </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200/80 dark:border-slate-800 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden flex flex-col justify-between group">
         <div className="absolute top-0 right-0 w-40 h-40 bg-gradient-to-br from-amber-400/10 dark:from-amber-400/5 to-transparent rounded-full blur-[30px] -mr-10 -mt-10 transition-transform duration-700 group-hover:scale-125"></div>
         <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-[0.02] dark:opacity-[0.05] mix-blend-overlay pointer-events-none"></div>
         <div className="absolute bottom-0 left-0 w-full h-[3px] bg-gradient-to-r from-amber-500 to-orange-500 scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"></div>
         
         <h3 className="font-display font-medium text-slate-800 dark:text-slate-100 text-lg mb-6 tracking-tight relative z-10">{t('chart_age_title')}</h3>
         <div className="h-[250px] w-full relative z-10">
            <ResponsiveContainer width="100%" height="100%">
               <BarChart data={ageDistData}>
                 <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                 <XAxis dataKey="range" axisLine={false} tickLine={false} tick={{ fontSize: isMobile ? 9 : 11, fill: '#94a3b8', fontWeight: '600' }} dy={10} />
                 <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#94a3b8' }} dx={-10} allowDecimals={false} />
                 <Tooltip cursor={{ fill: 'rgba(248, 250, 252, 0.1)' }} contentStyle={{ borderRadius: '1rem', border: '1px solid #f1f5f9', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.05)' }} />
                 <Bar dataKey="Jumlah" radius={[6, 6, 0, 0]} barSize={24}>
                    {ageDistData.map((entry, index) => <Cell key={`cell-${index}`} fill={ageColors[index % ageColors.length]} />)}
                 </Bar>
               </BarChart>
            </ResponsiveContainer>
         </div>
